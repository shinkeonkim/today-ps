...
int main() {
    ...
    Point current = getAveragePoint(ar, cnt);

    for1(0, gen) {
        Point farPoint = getFarestPoint(ar, cnt, current);
    ...
    Point farPoint = getFarestPoint(ar, cnt, current);
    double mx = getDistance(current, farPoint);

    ll chk = 0;
    for1(0, cnt2) {
        if(getDistance(current, notIn[i]) < mx) {
            chk++;
        }
    }
    if(chk > 1) {
